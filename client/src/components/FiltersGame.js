...
// import classesFilter from '../filters/classes';
// import numberTeachers from '../filters/number_teachers';
// import physicalActivity from '../filters/physical_activity';
...
const FiltersGame = props => {
  ...
  return (
    <React.Fragment>
      ...
      <FormGroup row>
        <CheckboxList
          title={classesFilter.title.label}
          checkboxes={classesFilter.checkboxes}
      ...
      <FormGroup row>
        {isEdit ? (
          <RadioList
            title={numberTeachers.title.label}
            selected={number_teachers[0]}
            radioButtons={numberTeachers.checkboxes}
            groupName={numberTeachers.title.name}
            handleChange={handleRadioButtonChange}
          />
        ) : (
          <CheckboxList
            title={numberTeachers.title.label}
            checkboxes={numberTeachers.checkboxes}
            selectedItems={number_teachers}
            handleChange={handleCheckboxChange}
          />
        )}
      </FormGroup> */}
      ...
      <FormGroup row>
        {isEdit ? (
          <RadioList
            title={physicalActivity.title.label}
            selected={physical_activity[0]}
            radioButtons={physicalActivity.checkboxes}
            groupName={physicalActivity.title.name}
            handleChange={handleRadioButtonChange}
          />
        ) : (
          <CheckboxList
            title={physicalActivity.title.label}
            checkboxes={physicalActivity.checkboxes}
            selectedItems={physical_activity}
            handleChange={handleCheckboxChange}
          />
        )}
      </FormGroup> */}
